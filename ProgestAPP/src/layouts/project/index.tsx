...
export default function Dashboard(props: { [x: string]: any }) {
  ...
  const getRoutes = (routes: RoutesType[]): JSX.Element[] => {
    const validLayouts = ['/project', '/purchase-order', '/change-order', '/invoice', '/receipt', '/pay-roll', '/material-request', '/reimbursement', '/service'];
  ...
  const shouldShowBox = currentPath.includes('/project/index');
  ...
  return (
    <Box>
		{shouldShowBox ? (
			...
			<Box mx='auto' p={{ base: '20px', md: '30px' }} pe='20px' maxW='lg'>
				<Switch>
					{getRoutes(routes)}
